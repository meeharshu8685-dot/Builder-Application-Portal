    ...
    >
      ...
      <section className="text-center mb-24 max-w-4xl mx-auto">
        <motion.h1
          ...
          className="text-4xl sm:text-5xl md:text-7xl font-extrabold leading-[1.2] sm:leading-[1.1] mb-6 sm:mb-8 font-heading text-white tracking-tight"
        >
          Not a big creator? <br />
          <span className="text-white/40">Not a problem.</span>
        </motion.h1>

        <motion.div variants={itemVariants} className="space-y-4 mb-10 sm:mb-12">
          <p className="text-lg sm:text-2xl text-[#9CA3AF] font-medium leading-relaxed px-4 sm:px-0">
            If you’re creating — you’re welcome here.
          </p>
          <p className="text-base sm:text-lg text-[#9CA3AF]/60 max-w-2xl mx-auto leading-relaxed px-4 sm:px-0">
            A place for people who are still learning, still experimenting, and still showing up.
          </p>
        </motion.div>

        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 justify-center px-6 sm:px-0">
          <button
            onClick={onNext}
            className="w-full sm:w-auto bg-[#7C7CFF] hover:bg-[#8e8eff] text-white font-bold py-4 px-10 rounded-full text-lg transition-all duration-300 shadow-[0_0_30px_rgba(124,124,255,0.3)] hover:shadow-[0_0_40px_rgba(124,124,255,0.5)] transform hover:-translate-y-1"
          >
            Start a collab
          </button>
          <button
            onClick={onNext}
            className="w-full sm:w-auto bg-white/5 hover:bg-white/10 text-white font-semibold py-4 px-10 rounded-full text-lg transition-all duration-300 border border-white/10"
          >
            I'm just exploring
          </button>
        </motion.div>
      ...
      <section className="w-full mb-32 max-w-3xl mx-auto p-8 sm:p-12 rounded-3xl sm:rounded-[2.5rem] bg-gradient-to-b from-white/[0.04] to-transparent border border-white/5">
        <motion.h2 variants={itemVariants} className="text-2xl sm:text-3xl font-bold mb-8 sm:mb-12 text-center text-white">
          You don't need:
        </motion.h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 mb-10 sm:mb-12">
          {[
            ...
            <motion.div
              key={i}
              variants={itemVariants}
              className="flex items-center gap-3 sm:gap-4 text-lg sm:text-xl text-[#9CA3AF]"
            >
              <span className="text-red-400/60 font-bold ml-1 sm:ml-2">✕</span>
              {item}
            </motion.div>
          ))}
        </div>
        <motion.p variants={itemVariants} className="text-center text-xl sm:text-2xl font-medium text-white/90">
          You just need <br className="sm:hidden" /> <span className="text-[#7C7CFF]">curiosity</span> and <span className="text-[#7C7CFF]">consistency</span>.
        </motion.p>
      </section>

      {/* 4. How Collab Works */}
      <section className="w-full max-w-5xl mb-24 px-4 sm:px-0">
        <motion.h2 variants={itemVariants} className="text-2xl sm:text-3xl font-bold mb-12 sm:mb-16 text-center text-white">
          How it works
        </motion.h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8">
          {[
            ...
            <motion.div key={i} variants={itemVariants} className="relative flex flex-col items-center text-center md:items-start md:text-left">
              <div className="text-5xl sm:text-6xl font-black text-white/5 absolute -top-8 sm:-top-10 left-1/2 md:left-0 transform -translate-x-1/2 md:translate-x-0">
                {item.step}
              </div>
              <h3 className="text-xl font-bold mb-2 sm:mb-3 text-white relative z-10">{item.title}</h3>
